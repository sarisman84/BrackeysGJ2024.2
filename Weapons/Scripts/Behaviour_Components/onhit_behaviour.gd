...
func apply_behaviour(args : Dictionary = {}) -> Dictionary:
	...
	if args.has("bullets"):
		...
		hurtbox.body_entered.connect(func(incoming_body : Variant): on_bullet_hit(hurtbox, weapon_manager,damage_multiplier, incoming_body))
		hurtbox.area_entered.connect(func(incoming_body : Variant): on_bullet_hit(hurtbox, weapon_manager,damage_multiplier, incoming_body))
	return args
